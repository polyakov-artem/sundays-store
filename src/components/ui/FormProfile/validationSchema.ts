...
export const shippingAddressValidator = Yup.object().shape({
...
export const billingAddressValidator = Yup.object().shape({
  [KEY_SAME_AS]: Yup.string(),
  [KEY_STREET]: Yup.string().when(KEY_SAME_AS, {
    is: (value: string) => !value,
    then: () => streetValidator,
    otherwise: () => Yup.string().notRequired(),
  }),
  [KEY_CITY]: Yup.string().when(KEY_SAME_AS, {
    is: (value: string) => !value,
    then: () => cityValidator,
    otherwise: () => Yup.string().notRequired(),
  }),
  [KEY_POSTAL_CODE]: Yup.string().when(KEY_SAME_AS, {
    is: (value: string) => !value,
    then: () => postalCodeValidator,
    otherwise: () => Yup.string().notRequired(),
  }),
  [KEY_COUNTRY]: Yup.string().when(KEY_SAME_AS, {
    is: (value: string) => !value,
    then: () => countryValidator,
    otherwise: () => Yup.string().notRequired(),
  }),
});
...
export const validationSchema = Yup.object().shape({
  ...
  [KEY_DATE_OF_BIRTH]: Yup.string()
    ...
    .required(inputErrors.required),
  [KEY_ADDRESSES]: Yup.object().shape({
    [KEY_SHIPPING]: Yup.array().of(shippingAddressValidator),
    [KEY_BILLING]: Yup.array().of(billingAddressValidator),
  }),
});
